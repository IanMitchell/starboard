...
export default async ({ bot }: CommandArgs) => {
	bot.onSlashCommand(command, async (interaction) => {
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(
				`I listen for ⭐️ reactions and repost the best messages to a starboard channel!\n\nTo get started, first run </log:999079924377845863> to setup the starboard channel, and then customize the amount of stars needed for messages to get posted with </amount:999079924377845861>.\n\nSource available on [GitHub](https://github.com/ianmitchell/starboard).`
			)
